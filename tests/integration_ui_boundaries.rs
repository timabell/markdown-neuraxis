...
use markdown_neuraxis::io;
use markdown_neuraxis::models::MarkdownFile;
  ...
  - Nested Item 2.1"#;
    ...
    let list_item_blocks: Vec<_> = snapshot
        ...
        .filter(|block| {
            matches!(
                block.kind,
                markdown_neuraxis::editing::BlockKind::ListItem { .. }
            )
        })
        .collect();
...
fn test_nested_list_anchor_hierarchy() {
  ...
  - Child Item 3"#;
    ...
    for block in &snapshot.blocks {
        if let markdown_neuraxis::editing::BlockKind::ListItem { .. } = block.kind {
