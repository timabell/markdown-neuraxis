...
impl Document {
    ...
    pub fn snapshot(&self) -> Snapshot {
        ...
        if let Some(ref tree) = self.tree {
            ...
            self.collect_render_blocks_recursive(root_node, &mut blocks, 0);
        }
    ...
    fn collect_render_blocks_recursive(
        ...
        match node_kind {
            "atx_heading" => {
                ...
                let content_range = self.extract_heading_content_range(&node);
                let anchor_id = self.find_anchor_for_range(&byte_range);
            ...
            "list_item" => {
                let marker = self.extract_list_marker(&node);
                let list_depth = self.calculate_list_depth(&node);
                let content_range = self.extract_list_item_content_range(&node);
                let anchor_id = self.find_anchor_for_range(&byte_range);
                ...
                for child in node.children(&mut cursor) {
                    self.collect_render_blocks_recursive(child, blocks, list_depth);
                }
            }
            "paragraph" => {
            ...
            "fenced_code_block" => {
                let lang = self.extract_code_fence_language(&node);
                let content_range = self.extract_code_fence_content_range(&node);
                let anchor_id = self.find_anchor_for_range(&byte_range);
            ...
            _ => {
                ...
                let mut cursor = node.walk();
                for child in node.children(&mut cursor) {
                    self.collect_render_blocks_recursive(child, blocks, current_depth);
                }
            }
        }
    }
    ...
    fn extract_heading_level(&self, node: &tree_sitter::Node) -> u8 {
        let text = self.buffer.slice_to_cow(node.byte_range());
    ...
    fn extract_heading_content_range(&self, node: &tree_sitter::Node) -> std::ops::Range<usize> {
        let byte_range = node.byte_range();
        let text = self.buffer.slice_to_cow(byte_range.clone());
    ...
    fn extract_list_marker(&self, node: &tree_sitter::Node) -> Marker {
        let byte_range = node.byte_range();
        let text = self.buffer.slice_to_cow(byte_range);
    ...
    fn calculate_list_depth(&self, node: &tree_sitter::Node) -> usize {
        let byte_range = node.byte_range();
        let text = self.buffer.slice_to_cow(byte_range);
    ...
    fn extract_list_item_content_range(&self, node: &tree_sitter::Node) -> std::ops::Range<usize> {
        let byte_range = node.byte_range();
        let text = self.buffer.slice_to_cow(byte_range.clone());
    ...
    fn extract_code_fence_language(&self, node: &tree_sitter::Node) -> Option<String> {
    ...
    fn extract_code_fence_content_range(&self, node: &tree_sitter::Node) -> std::ops::Range<usize> {
        let byte_range = node.byte_range();
        let text = self.buffer.slice_to_cow(byte_range.clone());
    ...
    fn find_anchor_for_range(&self, range: &std::ops::Range<usize>) -> AnchorId {
        ...
        best_anchor.unwrap_or_else(|| self.generate_temp_anchor_id(range))
    }

    /// Generate a temporary anchor ID for a range (fallback)
    fn generate_temp_anchor_id(&self, range: &std::ops::Range<usize>) -> AnchorId {
