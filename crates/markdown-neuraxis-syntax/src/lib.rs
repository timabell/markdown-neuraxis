...
mod tests {
    ...
    fn snapshot_test(rel_path: &str) {
        ...
        let crate_snap_dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("src/snapshots");
        ...
        settings.bind(|| {
            insta::assert_snapshot!(snapshot_name, insta_format_tree(&tree, 0));
        });
        ...
        assert_eq!(
            tree.text().to_string(),
            input,
            "{rel_path}: roundtrip failed"
        );
    }

    // Generated by build.rs - one test per .md file in snapshots/
    // Each snapshot test already verifies roundtrip (lossless parsing).
