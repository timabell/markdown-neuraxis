...
fn flatten_blocks(blocks: &[Block], source: &str, out: &mut Vec<(String, AnchorId)>) {
    for block in blocks {
        ...
        if let BlockContent::Children(children) = &block.content {
            flatten_blocks(children, source, out);
        }
    }
}
...
mod anchor_stability_tests {
    ...
    fn test_anchor_ids_stable_during_focus_cycles() {
        ...
        flatten_blocks(&initial_snapshot.blocks, &source, &mut initial_blocks);
        ...
        flatten_blocks(
            &after_focus_snapshot.blocks,
            &source,
            &mut after_focus_blocks,
        );
        ...
        for cycle in 0..3 {
            ...
            flatten_blocks(&cycle_snapshot.blocks, &source, &mut cycle_blocks);
    ...
    fn test_anchor_uniqueness_after_multiple_regenerations() {
        ...
        for cycle in 0..5 {
            ...
            flatten_blocks(&snapshot.blocks, &source, &mut blocks);
